...
export function createBackendApiClient(baseUrl: string) {
  ...
  async function requestBackend<T>(path: string, init?: RequestInit): Promise<T> {
    ...
    if (!response.ok) {
      ...
      throw new BackendApiError(data.error || 'Request failed', response.status, data);
    }
    return response.json();
  }
  function postBackend<T>(path: string, body: unknown = {}) {
  ...
  return { requestBackend, postBackend, streamBackendEvents, urlFor };
}
export const { requestBackend, postBackend } = createBackendApiClient('/api');
