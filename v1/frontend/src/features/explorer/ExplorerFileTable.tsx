...
const ParentFolderNavigation = createContext<(() => void) | undefined>(undefined);

// Keep navigation outside the data rows so filtering, sorting, and pagination cannot hide it.
function ExplorerTableBody({ children, ...props }: HTMLAttributes<HTMLTableSectionElement>) {
  const onNavigateParent = useContext(ParentFolderNavigation);
  return (
    <tbody {...props}>
      {onNavigateParent && (
        <tr className="ant-table-row" onDoubleClick={onNavigateParent}>
          <td />
          <td colSpan={5}>
            <button
...
export function ExplorerFileTable({
  ...
  return (
    <ParentFolderNavigation.Provider value={onNavigateParent}>
      <Table<ExplorerNode>
        components={{ body: { wrapper: ExplorerTableBody } }}
        ...
        locale={{
          emptyText: (
            <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="No files in this folder">
              <Button
                type="primary"
              ...
              <Dropdown
                ...
                menu={{
                  ...
                  items: [
                    ...
                    { key: 'cut', label: 'Cut' },
                    ...(node.kind === 'folder'
                      ? [{ key: 'paste', label: 'Paste into folder' }]
                      : []),
                    ...(node.kind === 'file'
                      ? [
      ...
      />
    </ParentFolderNavigation.Provider>
  );
}
